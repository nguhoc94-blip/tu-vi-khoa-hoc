...
def _generate_paid_with_openai(*, chart_json: dict[str, Any], request_id: str) -> tuple[bool, str]:
...
def unlock_demo_for_sender(*, sender_id: str, request_id: str) -> str:
    ...
    success, full_text = _generate_paid_with_openai(chart_json=chart_json, request_id=request_id)
    if success:
        store.set_status(
...
def generate_full_reading_direct(
    *,
    ...
    success, text = _generate_paid_with_openai(chart_json=chart_json, request_id=request_id)
    if success:
